...
test("Place Order", async({  page }) => {
    ...
    await login_page.loginToApp(dataset.username, dataset.password)
    ...
    await dashboard_page.searchProductAddToCart(dataset.product);
    ...
    await checkout_page.checkoutOrder('12','31', '123', dataset.username, "rahulshettyacademy")
    await expect(checkout_page.applied_coupon_label).toHaveAttribute("style","color: green;")
    await expect(checkout_page.shipping_address_label).toContainText(dataset.username)
    ...
    await expect(order_details_page.order_product_name).toContainText(dataset.product)
})